...
public:
     ...
     * 哈夫曼编码
     */
    TreeNode<T> *CreateHuffman(T a[], int n){
        TreeNode<T> **b = new TreeNode<T> *[n];
        TreeNode<T> *q = new TreeNode<T>(0);
        for(int i = 0; i < n; i++){
            b[i] = new TreeNode<T>(a[i]);
        }
        for (int i = 1; i < n; i++) {
            int k1 = -1, k2 = 0;
            for (int j = 0; j < n; j++) {
            ...
            TreeNode<T> *l = b[k1];
            TreeNode<T> *r = b[k2];
            TreeNode<T> *temp = new TreeNode<T>(l->val+r->val);
            temp->left = l;
            temp->right = r;
            l->parent = temp;
            r->parent = temp;
